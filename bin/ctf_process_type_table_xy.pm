...
# ------------
#   table_xy
#     Currently just the output from extrema.pl - first block of data
#     # comments
#     #     X:  <other>:
#         <column of x>  <column of y>
#     <any other blocks ignored>
#
########################################################################
...
########################################################################
# Required routine: ctf_read_table_xy
sub ctf_read_table_xy{
    ...
    while( ! defined($done) ){
        ...
        # -table_xy file starts like this
        if( $file_top =~ /# title\s+=.*# index\s+=.*# comment\s+=.*# x\s+=.*# y\s+=.*/i ||
            ( defined($file) &&
              $file =~ /extrema\.dat/ ) ){
            $ierr = 0;
            last;
    ...
    if( defined($args{VERBOSE}) ){
        print "$args{VERBOSE}ctf_process_type_table_xy\n";
...
########################################################################
# Required routine: ctf_plot_table_xy
sub ctf_plot_table_xy{
    my %args = (
