...
########################################################################
# Required routine: ctf_read_eap_history
sub ctf_read_eap_history{
    ...
    while( ! defined($done) ){
        ...
        # -history file starts like this
        # If see this and the file name matches, then stop
        if( $file_top =~ /^\s*n\s+run\s+cycle\s+time\s+dt/m &&
            ( ! defined($file) ||
              $file =~ /\-history/ ) ){
            $ierr = 0;
            last;
    ...
    if( defined($args{VERBOSE}) ){
        print "$args{VERBOSE}ctf_process_type_eap_history\n";
    ...
    while( ! defined($done) ){
        ...
        while( ! defined($done) ){
            ...
            # data (starts with numeric fields)
            if( $line =~ /^\s*(\d+)\s+(\d+)/ ){
...
########################################################################
# Required routine: ctf_plot_eap_history
sub ctf_plot_eap_history{
    my %args = (
    ...
    foreach $field ( @fields_all ){
        ...
        if( $field =~ /^(date:time|prbnm|job-id)$/ ){
            next;
        }
